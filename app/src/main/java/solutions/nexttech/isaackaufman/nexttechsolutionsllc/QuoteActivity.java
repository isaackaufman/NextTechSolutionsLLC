    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (name.equals("") || contact.equals("") || desc.equals("")) {
                    Toast.makeText(getApplicationContext(), "Please fill out all fields", Toast.LENGTH_SHORT).show();
                } else {
                    try {
                        String result = new SendEmailAsync().execute(name, contact, date, time, desc, method, "quote").get();
                        if (result == "success") {
                            Toast.makeText(getApplicationContext(), "Success! Expect an estimate within 12 hours!", Toast.LENGTH_LONG).show();
                        } else {
